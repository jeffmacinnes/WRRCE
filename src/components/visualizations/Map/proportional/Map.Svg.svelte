<script>
  ...
  $: features = $data.features.map((feature) => {
    ...
    let propCompliance = 0;
    if (match) {
      fill = colorScale(match.propCompliance);
      propCompliance = match.propCompliance;
    }
    feature.properties.fill = fill;
    feature.properties.propCompliance = propCompliance;

    // manually overwrite Belarus (370) and Kosovo (370)
    if (["370", "347"].includes(feature.properties.ccode)) {
      feature.properties.propCompliance = null;
    }
  ...
  const getColor = (ccode) => {
    ...
    return match ? colorScale(match.propCompliance) : landColor;
  };
